...
import { DOCUMENT_TYPES, normalize } from "./lib/config";
...
async function runValidation(
  ...
  // 1. Completeness
  const missing = ["documentNumber", "businessName", "issueDate"].filter((k) => !fields[k]?.trim());
  checks.push({
    check: "Completeness",
    status: missing.length === 0 ? "PASSED" : "FAILED",
    detail:
      missing.length === 0
        ? "Document number, business name and issue date extracted."
        : `Missing fields: ${missing.join(", ")}.`,
  });

  // 2. Format
  const numOk = documentNumber ? /^[A-Z0-9][A-Z0-9/._-]*$/i.test(documentNumber.trim()) : false;
  const dateOk = issueDate !== null;
  checks.push({
    check: "Format",
    status: numOk && dateOk ? "PASSED" : "WARNING",
    detail:
      !numOk && !dateOk
  ...
  checks.push({
    ...
    status: nameOk ? "PASSED" : (businessName ? "FAILED" : "N/A"),
    detail:
      businessName && !nameOk
        ...
        : businessName
          ...
          : "No business name extracted to compare.",
  });
    ...
    detail:
      dupByHash.length > 0
        ...
        : dupByNumber.length > 0
          ...
          : "No duplicate detected (exact hash or document number).",
  });
...
export const myDocuments = query({
  ...
  handler: async (ctx: QueryCtx) => {
    ...
    const filtered = all.filter(
      (d) => !d.applicationId || allowedAppIds.has(d.applicationId),
    );
