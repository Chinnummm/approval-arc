...
export async function assertSubmitReady(ctx: MutationCtx, applicationId: Id<"applications">) {
  ...
  const rule =
    rules
      .filter((r) => r.ruleId === app.ruleId && r.verificationStatus === "ACTIVE")
  ...
  for (const key of rule.requiredDocuments) {
    const ok = docs.some(
      (d) =>
        ...
        d.documentType === key &&
        d.fieldsConfirmed &&
        ["PASSED", "PARTIAL"].includes(d.validationStatus),
    );
    if (!ok)
...
export const submitApplication = mutation({
  ...
  handler: async (ctx: MutationCtx, args) => {
    ...
    const { app } = await assertSubmitReady(ctx, args.applicationId);
    ...
    await ctx.db.insert("governmentSubmissions", {
      ...
      submittedAt: now,
      lastSyncAt: now,
      rawResponse: "MOCK_GATEWAY: received",
      isSimulation: true,
    } as never);
