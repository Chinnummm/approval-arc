...
export function ApplicantApplicationDetail() {
  ...
  const orgDocs = detail.documents;
  const appDocs = orgDocs.filter((d) => d.applicationId === app._id);
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[1fr_360px]">
        <div className="min-w-0 space-y-6">
          ...
          <Section title="Government gateway" description="Where this application stands with the authority.">
            <div className="p-4">
              <GatewayPanel submission={detail.governmentSubmission} applicationStatus={app.status} />
            </div>
          </Section>
          ...
          <Section title="Documents for this approval">
            {appDocs.length === 0 ? (
              <p className="px-4 py-6 text-xs text-muted-foreground">
                No documents attached. Upload documents for this approval from the Document Vault.
              </p>
            ) : (
              <ul className="divide-y divide-border">
                {appDocs.map((d) => (
