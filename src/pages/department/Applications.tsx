  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {["", "SUBMITTED", "UNDER_REVIEW", "QUERY_RAISED", "DECISION_PENDING", "APPROVED", "REJECTED"].map((s) => (
...
export function DepartmentApplicationReview() {
  ...
  const { application: app, rule, evalInfo, queries, inspections, governmentSubmission, sla, readiness, documents } = detail;
  const appDocs = documents.filter((d) => d.applicationId === app._id);
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[1fr_380px]">
              ...
              <div className="border-t p-4">
                {inspections.filter((ins) => ins.status === "SCHEDULED").map((ins) => (
                  <div key={ins._id} className="flex items-center justify-between gap-2">
                    ...
                    <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => action("complete inspection", () => completeInspection({ inspectionId: ins._id, outcome: "PASSED", outcomeNotes: "Inspection completed." }))}>
                      Mark complete
                    </Button>
                  </div>
                ))}
        ...
        <div className="space-y-6">
          ...
          <Section title="Gateway" padded>
            <GatewayPanel submission={governmentSubmission} applicationStatus={app.status} />
          </Section>
